...
object Veldpaus {
  ...
  def veldpaus(points: Traversable[(Vec3D, Vec3D)], tol: Double = 1.0e-10): Result = {
    ...
    val G: DenseMatrix64F = {
      ...
      while (index < xprime.length) {
        ...
        CommonOps.multAddTransB(x, y, g) // g = g + x * y^T
        index += 1
      }
  ...
  private def adjoint3(m: DenseMatrix64F, r: DenseMatrix64F = new DenseMatrix64F(3, 3)): DenseMatrix64F = {
    ...
    def cross(a: DenseMatrix64F, acol: Int, b: DenseMatrix64F, bcol: Int, destCol: Int) {
      r.set(0, destCol, a.unsafe_get(1,acol) * b.unsafe_get(2,bcol) - a.unsafe_get(2,acol) * b.unsafe_get(1,bcol))
      r.set(1, destCol, a.unsafe_get(2,acol) * b.unsafe_get(0,bcol) - a.unsafe_get(0,acol) * b.unsafe_get(2,bcol))
      r.set(2, destCol, a.unsafe_get(0,acol) * b.unsafe_get(1,bcol) - a.unsafe_get(1,acol) * b.unsafe_get(0,bcol))
    }
    cross(m, 1, m, 2, 0)
    cross(m, 2, m, 0, 1)
    cross(m, 0, m, 1, 2)
